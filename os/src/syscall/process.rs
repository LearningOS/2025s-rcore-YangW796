...
use crate::{
    ...
    task::{
        change_program_brk, check_vpn_rw, check_vpn_used, create_new_map_area, current_user_token, delete_map_area, exit_current_and_run_next, get_current_syscall_count, suspend_current_and_run_next, vaddr_to_paddr}, 
    timer::get_time_us
};
...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let mut dst=translated_byte_buffer(current_user_token(),ts as *const u8,size_of::<TimeVal>());
    ...
    for buf in dst.iter_mut(){
        ...
        unsafe{
            copy_nonoverlapping(
                time_val_bytes.as_ptr().add(offset), buf.as_mut_ptr(),copy_len);
        }
        offset+=copy_len;
...
pub fn sys_trace(trace_request: usize, id: usize, data: usize) -> isize {
    ...
    match trace_request {
        0 => {
            if id>0x0000003fffffffff{return -1;}
            let vpn=VirtAddr::from(id).floor();
            if !check_vpn_rw(vpn, 0){return -1;}
        ...
        1 => {
            if id>0x0000003fffffffff{return -1;}
            let vpn=VirtAddr::from(id).floor();
            if !check_vpn_rw(vpn, 1){return -1;}
            let p_addr:PhysAddr = vaddr_to_paddr(current_user_token(),id);
...
pub fn sys_mmap(start: usize, len: usize, prot: usize) -> isize {
    ...
    if check_vpn_used(start_vpn, end_vpn){
        return -1;
    }

    let mut entry:MapPermission=MapPermission::empty();
    if prot & 1 != 0 {entry.insert(MapPermission::R);}
    if prot & 2 != 0 {entry.insert(MapPermission::W);}
    if prot & 4 != 0 {entry.insert(MapPermission::X);}
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    delete_map_area(start_vpn, end_vpn)
